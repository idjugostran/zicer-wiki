...
def main():
    ...
    # Cheap stats line, computed once here (not on every skill/bot query):
    # source count and the most recent `updated` date across all pages.
    # ISO YYYY-MM-DD strings sort lexicographically, so plain max() works.
    source_count = sum(1 for p in pages if p["category"] == "Sources")
    last_updated = max((p["updated"] for p in pages if p["updated"]), default="")

    out = [f"# Wiki Index — {domain}".rstrip(), "",
           "<!-- Generated by bin/generate-index.py. Do not edit by hand. -->",
           f"**Sources:** {source_count} · **Last updated:** {last_updated}", ""]
    for cat in ordered:
