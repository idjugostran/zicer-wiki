...
#   2. Restart the Hermes gateway, if running and something was removed.
#   3. Restart the Hermes desktop app, if running and something was removed.
#
# Scope: this only undoes the *install* — the local checkout and the Hermes
# registration. It does not touch Hermes's own logs/conversation history
# (those may still mention Zicer from past replies), and it never touches
# the wiki's source repo (github.com/idjugostran/zicer-wiki) or any dev
# clone of it — only the installed copy on *this* machine.
#
...
elif [[ ! -d "$INSTALL_DIR/.git" ]] || [[ ! -d "$INSTALL_DIR/skill/zicer-wiki-context" ]]; then
  echo "  WARNING: $INSTALL_DIR doesn't look like a zicer-wiki checkout" >&2
  echo "           (missing .git or skill/zicer-wiki-context) - refusing to delete it." >&2
...
if [[ "$DIR_REMOVED" -eq 1 || "$CONFIG_CHANGED" -eq 1 ]]; then
  CHANGED=1
else
  CHANGED=0
fi

echo "== 2. Restart Hermes gateway (drop the removed skill) =="
...
echo "== 3. Restart Hermes desktop app (drop the removed skill) =="
if [[ "$CHANGED" -eq 0 ]]; then
  echo "  Skipped: nothing changed this run, no need to restart"
