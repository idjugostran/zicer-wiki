...
│   ├── overview.md
│   └── pages/*.md
└── skill/zicer-wiki-context/SKILL.md   <- this file
```
...
3. **Synthesize an answer grounded in what you read, citing in chat-safe
   ...
   - **Disclose that this skill answered.** End the reply with a short,
     ...
     (zicer-wiki-context)` — no brackets/links, just text, so it can't trip
...
(zicer-wiki-context)` disclosure line — if it's missing, add it before
sending.
