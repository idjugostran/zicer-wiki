...
## Conventions
- raw/ is immutable — skills never modify it. It IS committed: `raw/<ID>.*` goes into
  the same commit as the source page it backs, because citation locators resolve against
  it. Deduplicated transcripts live in the session scratchpad and are never committed.
  A video that is skipped (see `config/skip.txt`) leaves nothing in raw/
